...
/**
 * Operator controller bindings (Port 1). State-machine management, safety
 * overrides, and shooter
 * pivot manual control. Mechanism actions route through the
 * {@link Superstructure}.
 */
public final class OperatorControls {
         ...
         * @param shooterPivot                shooter pivot subsystem (for manual
         *                                    override + homing, bypasses
         *                                    Superstructure)
         * @param climber                     climber subsystem (for climb safety
         *                                    interlock)
         ...
         * @param climbApproachCommandFactory factory that creates a fresh command to
         *                                    pathfind to the
         *                                    climb approach point
         * @param climbEntryCommandFactory    factory that creates a fresh command to
         *                                    pathfind from the
         *                                    approach point into the climb entry point
         * @param climbLaneSupplier           supplier for currently selected climb lane
         * @param drivetrain                  swerve drivetrain subsystem (for
         *                                    auto-climb final nudge)
         */
        public static void configure(
                        ...
                        Supplier<ClimbLane> climbLaneSupplier,
                ...
                operator
                                ...
                                .onTrue(Commands.defer(
                                                () -> {
                                                        SwerveRequest.ApplyRobotSpeeds finalForwardRequest = new SwerveRequest.ApplyRobotSpeeds();
                                                        SwerveRequest.ApplyRobotSpeeds rightPreEntryLeftRequest = new SwerveRequest.ApplyRobotSpeeds();
                                                        SwerveRequest.ApplyRobotSpeeds rightPostEntryForwardRequest = new SwerveRequest.ApplyRobotSpeeds();

                                                        ClimbLane selectedLane = climbLaneSupplier.get();

                                                        double finalForwardSpeedMps = Math.max(0.05,
                                                                        Math.abs(ClimbConstants
                                                                                        .getAutoClimbFinalForwardSpeedMps()));
                                                        double finalForwardDistanceMeters = Math.max(0.0,
                                                                        Math.abs(ClimbConstants
                                                                                        .getAutoClimbFinalForwardDistanceMeters()));
                                                        double postMoveSettleSeconds = Math.max(0.0,
                                                                        ClimbConstants.getAutoClimbPostMoveSettleSeconds());
                                                        double finalForwardTimeSec = finalForwardDistanceMeters
                                                                        / finalForwardSpeedMps;

                                                        double rightPreEntryLeftSpeedMps = Math.max(0.05,
                                                                        Math.abs(ClimbConstants
                                                                                        .getAutoClimbRightPreEntryLeftSpeedMps()));
                                                        double rightPreEntryLeftDistanceMeters = Math.max(0.0,
                                                                        Math.abs(ClimbConstants
                                                                                        .getAutoClimbRightPreEntryLeftDistanceMeters()));
                                                        double rightPreEntryLeftTimeSec = rightPreEntryLeftDistanceMeters
                                                                        / rightPreEntryLeftSpeedMps;

                                                        double rightPostEntryForwardSpeedMps = Math.max(0.05,
                                                                        Math.abs(ClimbConstants
                                                                                        .getAutoClimbRightPostEntryForwardSpeedMps()));
                                                        double rightPostEntryForwardDistanceMeters = Math.max(0.0,
                                                                        Math.abs(ClimbConstants
                                                                                        .getAutoClimbRightPostEntryForwardDistanceMeters()));
                                                        double rightPostEntryForwardTimeSec = rightPostEntryForwardDistanceMeters
                                                                        / rightPostEntryForwardSpeedMps;
                                                        ...
                                                        return Commands.sequence(
                                                                        ...
                                                                        Commands.either(
                                                                                        drivetrain
                                                                                                        .applyRequest(() -> rightPostEntryForwardRequest
                                                                                                                        .withSpeeds(
                                                                                                                                        new ChassisSpeeds(
                                                                                                                                                        rightPostEntryForwardSpeedMps,
                                                                                                                                                        0.0,
                                                                        ...
                                                                        drivetrain
                                                                                        .applyRequest(() -> finalForwardRequest
                                                                                                        .withSpeeds(
                                                                                                                        new ChassisSpeeds(
                                                                                                                                        finalForwardSpeedMps,
                                                                                                                                        0,
                ...
                operator
                                ...
                                .onTrue(Commands.runOnce(
                                                () -> superstructure.setWantedSuperState(WantedSuperState.UNJAM)))
                ...
                operator.povLeft().onTrue(Commands.runOnce(() -> {
                        ...
                        double currentRpm = ShooterInterpolationTable.getRPM(Meters.of(tuningDistanceMeters)).in(RPM);
                ...
                new Trigger(() -> operator.start().getAsBoolean() && operator.back().getAsBoolean())
                                .onTrue(Commands.sequence(
                                                Commands.runOnce(() -> superstructure
                                                                .setWantedSuperState(WantedSuperState.CLIMB)),
                                                climber.extendCommand()));
                ...
                operator
                                ...
                                .onTrue(Commands.sequence(
                                                ...
                                                Commands.runOnce(() -> superstructure
                                                                .setWantedSuperState(WantedSuperState.IDLE))));
        }
