 ...
 *
 * The setpoint is computed from the distance to the hub using interpolation
 * tables.
 *
...
public class ShooterSetpoint {
     ...
     *
     * Queries the interpolation tables for RPM and pivot angle, then validates
     * that the resulting angle is within the pivot's range of motion.
     *
    ...
    /**
     * Whether this setpoint is valid (achievable by the hardware).
     * If the setpoint exceeds hardware limits, it's marked invalid and the
     * robot should NOT fire.
     */
    ...
    /**
     * A fixed "fender shot" setpoint for shooting while pressed up against the hub.
     * These values bypass distance-based interpolation for a known reliable shot.
     */
