...
public final class ShooterFactory {
    ...
    /**
     * Ferrying with {@link ShooterConstants#FERRY_SHOT_SPEED} & {@link ShooterConstants#FERRY_SHOT_PIVOT_ANGLE}
     *
     * @param shooter      flywheel subsystem
     * @param shooterPivot pivot subsystem
     * @param indexer      indexer subsystem for feeding
     * @return command that aims and shoots immediately
     */
    public static Command ferryShoot(
            ShooterSubsystem shooter,
            ShooterPivotSubsystem shooterPivot,
            IndexerSubsystem indexer) {
        return shooter
                    .holdRPMCommand(ShooterConstants.FERRY_SHOT_SPEED)
                    .alongWith(
                            shooterPivot.goToAngleCommand(ShooterConstants.FERRY_SHOT_PIVOT_ANGLE),
                            Commands.waitUntil(() -> shooter.isReady() && shooterPivot.isAtTarget())
                                    .andThen(indexer.feedCommand()))
                    .withName("ShooterFactory Ferry Shot");
    }
