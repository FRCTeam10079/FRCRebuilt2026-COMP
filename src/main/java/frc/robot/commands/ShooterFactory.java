 ...
 * (flywheel RPM, pivot angle, heading alignment, setpoint validity)
 * are checked in exactly one place.
 *
 * The core shooting sequence is:
 * 1. A Supplier<ShooterSetpoint> provides the current distance-based setpoint
 * (RPM + angle)
 * 2. The flywheel and pivot track that setpoint continuously
...
public final class ShooterFactory {
         ...
         *
         * Conditions:
         * - Setpoint is valid (non-null, reasonable distance)
         * - Flywheel RPM within ON_TARGET_RPM_PERCENT of target
         * - Pivot angle within SHOOTING_TOLERANCE_DEGREES deg of target
        ...
        /**
         * Spin up the flywheel and move the pivot to track a dynamic setpoint.
         * Does NOT feed - just prepares the shooter. Useful for pre-spinning
         * while driving toward a scoring position.
         *
         ...
         *
         * Simultaneously:
         * - Spins up flywheel to setpoint RPM
         * - Tracks pivot angle from setpoint
         * - Waits until isOnTarget() is true, then feeds the indexer
         *
         * The driver holds this command while aiming the drivetrain at the hub
         * (via heading lock or manual rotation). When all conditions align, the
         * ball is automatically fed.
         *
        ...
        public static Command shoot(
                ...
                return aimAndSpinUp(setpointSupplier, shooter, shooterPivot)
                                .alongWith(
                                                Commands.waitUntil(
                                                                () -> isOnTarget(setpointSupplier, shooter,
                                                                                shooterPivot, headingOnTarget))
                                                                .andThen(indexer.feedCommand()))
                                .withName("ShooterFactory Shoot");
        }

        /**
         * Fender shot - fixed RPM and angle, no distance calculation needed.
         * Good for scoring from right in front of the hub.
         *
         * @param shooter      flywheel subsystem
         * @param shooterPivot pivot subsystem
        ...
        public static Command fenderShot(
                ...
                return shooter
                                ...
                                .alongWith(
                                                shooterPivot.goToAngleCommand(
                                                                ShooterConstants.FENDER_SHOT_PIVOT_DEGREES),
         ...
         *
         * Same as shoot() but with a timeout for auto routines.
         * The heading check is always true in auto (the robot should already
         * be aimed by the trajectory).
         *
         * @param setpointSupplier dynamic setpoint
         * @param shooter          flywheel subsystem
         * @param shooterPivot     pivot subsystem
         ...
         * while the driver controls translation.
         *
        ...
        public static Command aimAtHub(
                ...
                return drivetrain
                                .headingLockedDriveCommand(xInput, yInput, headingSupplier, maxVelocity,
                                                maxAngularVelocity)
